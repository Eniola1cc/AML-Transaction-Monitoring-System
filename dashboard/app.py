...
st.write(
    "Rule-based and machine-learning alert prioritisation system for suspicious transaction monitoring."
)
...
def load_data():
    ...
    alert_queue_path = "reports/prioritised_alert_queue.csv"
    if os.path.exists(alert_queue_path):
        alert_queue = pd.read_csv(alert_queue_path)
    else:
        st.warning(
            "Prioritised alert queue file not found. Showing summary reports only."
        )
...
if alert_queue.empty:
    st.info(
        "No detailed alert queue file available yet. Add reports/prioritised_alert_queue.csv to enable explorer mode."
    )
else:
    ...
    st.dataframe(filtered_alerts[available_cols].head(1000), width="stretch")
    st.caption(
        "Note: The dashboard displays the first 1,000 filtered alerts for performance."
    )
